...
def prepare_data(
    ...
    train_transforms = Compose([
        # Load npy file from the "image" key.
        Lambdad(keys=["image"], func=lambda x: np.load(x) if isinstance(x, str) else x),
        # Ensure channel-first data format.
        EnsureChannelFirstd(keys=["image"]),
        # Attach fixed metadata.
        Lambdad(keys="top_region_index", func=lambda _: [0, 1, 0, 0]),
        Lambdad(keys="bottom_region_index", func=lambda _: [0, 0, 1, 0]),
        Lambdad(keys="spacing", func=lambda _: [1.2, 1.2, 2.0]),
    ])
...
def train_one_epoch(
    ...
    for train_data in train_loader:
        ...
        top_region_index_tensor = torch.tensor(train_data["top_region_index"]).to(device)
        bottom_region_index_tensor = torch.tensor(train_data["bottom_region_index"]).to(device)
        spacing_tensor = torch.tensor(train_data["spacing"]).to(device)
...
def diff_model_train(env_config_path: str, model_config_path: str, model_def_path: str, num_gpus: int, amp: bool = True) -> None:
    ...
    if not hasattr(args, "csv_data_list"):
        args.csv_data_list = args.json_data_list
...
if __name__ == "__main__":
    ...
    parser.add_argument("--env_config", type=str, default="./configs/environment_maisi_diff_model_train.json",
                        help="Path to environment configuration file")
    parser.add_argument("--model_config", type=str, default="./configs/config_maisi_diff_model_train.json",
    ...
    parser.add_argument("--csv_data_list", type=str, default="/data3/wyh/mri/prostate_dataset/train_encode/encoded_info.csv",
                        help="Path to CSV file containing data paths")
    parser.add_argument("--embedding_base_dir", type=str, default="/data3/wyh/mri/prostate_dataset/train_encode",
