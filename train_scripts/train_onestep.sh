...
export PATH_TO_CSV_DATA_LIST="/data1/yuhan/mri/prostate_dataset/maisi_train_encode/encoded_info.csv"
export PATH_TO_EMBEDDING_BASE_DIR="/data1/yuhan/mri/prostate_dataset/maisi_train_encode"
...
export ONESTEP_TIMESTEP=999 # Timestep for the one-step prediction
export MIXED_PRECISION="fp16" # "no", "fp16", "bf16"

export OUTPUT_DIR="$LOTUS_DIR/output/train-onestep-finetuned-bsz$(($TRAIN_BATCH_SIZE * ${#CUDA_DEVICE_IDS_CONCAT} * $GRADIENT_ACCUMULATION_STEPS))" # Generic name
...
echo "Starting One-Step Training..."
...
if [ ! -f "$ACCELERATE_CONFIG_FILE" ]; then
    ...
    # You might want to create a default one here or guide the user.
    # For example, for a single GPU (if CUDA_DEVICE_IDS_CONCAT="0"):
    # if [ "$NUM_GPUS" -eq 1 ] && [ ! -f "accelerate_configs/0.yaml" ]; then
    #   echo "Attempting to create a default single-GPU accelerate config at accelerate_configs/0.yaml..."
    #   mkdir -p accelerate_configs
    #   cat <<EOL > accelerate_configs/0.yaml
    ...
    # use_cpu: false
    # EOL
    #   export ACCELERATE_CONFIG_FILE="accelerate_configs/0.yaml"
    # else
    #   exit 1
    # fi
    # The above example attempts to create a default. For multi-GPU, it's more complex.
    # For now, we'll just error out if not found.
    exit 1
fi

accelerate launch --config_file=$ACCELERATE_CONFIG_FILE --mixed_precision=$MIXED_PRECISION \
  --main_process_port="29501" \
  "$LOTUS_DIR/train_onestep.py" \
  ...
  --tracker_project_name="train_onestep_finetune" \
  --num_gpus=$NUM_GPUS \
